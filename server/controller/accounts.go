...
} //Receive id, query the account information in the database according to id, return json
...
func (c *Controller) AddAccount(ctx *gin.Context) {
	...
	account.ID = lastID
	ctx.JSON(http.StatusOK, account)
} //Receive new account information (json),
// verify legitimacy,
//then insert the account information
//into the database and return the inserted account information to the client
...
func (c *Controller) UpdateAccount(ctx *gin.Context) {
	id := ctx.Param("id") //Get the value named “id” from the request's URL parameters
	aid, err := strconv.Atoi(id)
	if err != nil {
		httputil.NewError(ctx, http.StatusBadRequest, err)
		return
	}
	var updateAccount model.UpdateAccount //Stores the updated account information extracted from the request
	if err := ctx.ShouldBindJSON(&updateAccount); err != nil {
		httputil.NewError(ctx, http.StatusBadRequest, err)
		return
	}
	account := model.Account{ //stores the updated account
		ID:   aid,
